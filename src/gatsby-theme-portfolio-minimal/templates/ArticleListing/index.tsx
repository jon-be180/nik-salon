...
export default function ArticleListingTemplate(
  ...
  function handleLoadMoreButtonClick(
    ...
    if (
      selectedArticlesNumber &&
      selectedArticlesNumber >= incrementedArticleNumber
    ) {
      setShownArticlesNumber(incrementedArticleNumber);
    } else if (
      !selectedArticlesNumber &&
      articlesNumber >= incrementedArticleNumber
    ) {
      setShownArticlesNumber(incrementedArticleNumber);
    }
  }
        ...
        >
          ...
          <div className={classes.Listing}>
            {articles
              .filter(
                (article) =>
                  !filterSelected || selectedArticleIds.includes(article.id)
              )
...
function extractFilterOptions(articles: ArticleTemplateData[]): FilterOption[] {
  ...
  return filterOptions.sort((a, b) =>
    a.relatedArticleIds.length > b.relatedArticleIds.length ? -1 : 1
  );
}
